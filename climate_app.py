...
# -----------------------------------
# 🌐 OZONE LAYER STATS (2025 + HISTORICAL)
# -----------------------------------
st.header("🌀 Ozone Layer Statistics (2025 & Historical)")
...
fig_ozone = px.line(ozone_data, x='Year', y='Ozone Concentration (Dobson Units)', markers=True,
                    title='Global Ozone Layer Recovery Trend')
st.plotly_chart(fig_ozone, use_container_width=True)

# -----------------------------------
# 💨 GAS EMISSIONS IN EARTH’S ATMOSPHERE
# -----------------------------------
st.header("🌫️ Atmospheric Gas Composition & Emissions")
gas_data = pd.DataFrame({
    'Gas': ['Nitrogen (N₂)', 'Oxygen (O₂)', 'Argon (Ar)', 'Carbon Dioxide (CO₂)', 'Methane (CH₄)', 'Ozone (O₃)'],
    'Percentage': [78.08, 20.95, 0.93, 0.04, 0.0002, 0.000007]
})
fig_gas_bar = px.bar(gas_data, x='Gas', y='Percentage', title='Gas Composition in Earth’s Atmosphere')
fig_gas_pie = px.pie(gas_data, names='Gas', values='Percentage', title='Atmospheric Gas Distribution')
st.plotly_chart(fig_gas_bar, use_container_width=True)
st.plotly_chart(fig_gas_pie, use_container_width=True)

# -----------------------------------
# 🧊 GLACIER STATISTICS (MELTING TRENDS)
# -----------------------------------
...
fig_glacier = px.area(glacier_data, x='Year', y='Global Glacier Volume Index',
                      title='Decline in Global Glacier Volume Over Time')
st.plotly_chart(fig_glacier, use_container_width=True)

# -----------------------------------
# 🎨 UI Enhancements: Animated Expander Sections
# -----------------------------------
